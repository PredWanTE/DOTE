...
def jar_gurobi_wrapper(cplex_path, args, all_output=True):
    ...
    with open('param.tmp', 'w') as file:
        for k in range(len(args)):
            param = args[k]
            if k == 0 or k == (len(args) - 1) or k == (len(args) - 2) or (compute_opts and k == (len(args) - 4)):
                continue
            if ((k == (len(args) - 4) or k == (len(args) - 3)) and param == ""): param = "0 1 0.0"
            file.write(str(param) + '\n')

    process = Popen(['java.exe'] + ['-jar'] + [args[0]] + ['param.tmp'], stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
    stdout = stdout.decode("utf-8")
    stderr = stderr.decode('utf-8')
...
def get_opt_cplex(props, tm, next_tm=None, opt_function="MAXUTIL", combine=None, use_cplex=False, idx=0, path=None):
    ...
    if use_cplex:
        ...
        args_java += [idx]
        args_java += [path]
        res = jar_gurobi_wrapper(props.gurobi_path, args_java)
...
def get_data_dir_latent(props, is_test):
    postfix = "test" if is_test else "train"
    return props.hist_location + "/%s/latent/%s/" % (props.ecmp_topo, postfix)
